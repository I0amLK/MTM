...
| MTM-015 | A0, A1, A3, A4, A5 for non-overwriting secret initialization, redacted capability diagnostics, distinct authorization/submission outcome classification, bounded same-run caller recovery, permanent current-source capability regression, real client/restart/independent-instance validation, required-LaTeX workflow non-regression, and copied-state rollback/recutover |
