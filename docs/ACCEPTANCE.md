...
| MTM-015 | A0, A1, A3, A4, A5 for non-overwriting secret initialization, redacted capability diagnostics, distinct authorization/submission outcome classification, bounded same-run caller recovery, permanent current-source capability regression, RFC 9728 `/mcp` OAuth protected-resource interoperability, clean real web-client capability roundtrips, required-LaTeX workflow non-regression, copied-state compatibility, and actual preview.1 rollback/preview.2 recutover |
