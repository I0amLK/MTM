...
class GovernanceTestCase(unittest.TestCase):
    ...
    def test_mtm014_native_permission_contract_is_frozen(self) -> None:
        ...
        self.assertTrue(d5a_progress["runtime_request_permissions_changed"])
        ...
        self.assertTrue(bridge["legacy_unsupported_preserved"])
        self.assertTrue(bridge["modern_without_form_elicitation_unsupported_preserved"])
        self.assertTrue(bridge["trusted_implicit_permission_requires_no_prompt"])
        self.assertTrue(bridge["non_intrinsic_permission_requires_no_prompt"])
        self.assertTrue(bridge["dangerous_compatibility_preserved"])
        self.assertFalse(bridge["decline_mints_grant"])
        self.assertFalse(bridge["prompt_contains_raw_command"])
        self.assertFalse(bridge["grant_response_contains_raw_command"])
        self.assertFalse(bridge["workflow_authority_inherited"])
        self.assertTrue(bridge["safe_trusted_dangerous_servers_exit_cleanly"])
        self.assertFalse(bridge["raw_oauth_token_recorded"])
        self.assertFalse(bridge["raw_request_state_recorded"])
        self.assertFalse(bridge["raw_grant_id_recorded"])
