...
def score(value: str) -> int:
    parsed = int(value)
    if not 1 <= parsed <= 5:
        raise argparse.ArgumentTypeError("score must be 1..5")
    return parsed
...
def main() -> int:
    ...
    if pair is None or pair.get("protocol2") is None or pair.get("protocol3") is None:
        raise SystemExit("both treatment runs must exist before blind scoring")
    if pair.get("blind_evaluation") is not None:
        raise SystemExit("blind score already recorded")
    pair["blind_evaluation"] = {
        "status": "scored",
        "treatment_hidden_until_scores_frozen": True,
        "evaluator_id_hash": arguments.evaluator_id_hash,
        "artifacts": {
            "A": {"logic_completeness": arguments.a_logic, "readability": arguments.a_readability, "research_efficiency": arguments.a_efficiency},
            "B": {"logic_completeness": arguments.b_logic, "readability": arguments.b_readability, "research_efficiency": arguments.b_efficiency}
        },
