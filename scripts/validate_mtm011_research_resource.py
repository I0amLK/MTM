...
REPORT = ROOT / "records/evidence/MTM-011/research-resource.json"
HARNESS = ROOT / "scripts" / "run_mtm011_research_resource.py"
...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") != "1.0.0" or payload.get("milestone") != "MTM-011":
        raise ValueError("unexpected MTM-011 resource evidence identity")
    ...
    if payload.get("harness_sha256") != sha256_file(HARNESS):
        raise ValueError("resource evidence is stale for the MTM-011 wrapper")
    if payload.get("measurement_harness_sha256") != sha256_file(MEASUREMENT_HARNESS):
        raise ValueError("resource evidence is stale for the measurement harness")
    ...
    if not isinstance(protocol2, dict) or not isinstance(protocol3, dict):
        raise ValueError("resource evidence requires protocol2 and protocol3")
    ...
    if not isinstance(checks, dict) or len(checks) < 8 or not all(checks.values()):
        raise ValueError("one or more MTM-011 resource checks failed")
    if protocol2.get("research_view_bytes") != 0:
        raise ValueError("protocol 2 must not expose the protocol-3 research view")
